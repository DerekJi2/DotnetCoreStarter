...
namespace Dcs.Core.Reflection
{
    internal static class ReflectionHelper
    {
        ...
        /// <returns></returns>
        internal static List<string> GetEntities(Type entityType)
        {
            var types = AppDomain.CurrentDomain.GetAssemblies().SelectMany(x => x.GetTypes())
                .Where(x => entityType.IsAssignableFrom(x) && !x.IsInterface && !x.IsAbstract)
                .ToList();

            var result = types.Select(x => x.Name).ToList();
            return result;
        }
    }
}
